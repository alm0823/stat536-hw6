...
{\ttfamily\noindent\color{warningcolor}{Warning in int\_abline(a = a, b = b, h = h, v = v, untf = untf, ...): "{}add"{} is not a graphical parameter}}\end{kframe}
...
\hlstd{year} \hlkwb{<-} \hlkwd{data.frame}\hlstd{(}\hlkwd{replicate}\hlstd{(}\hlnum{10}\hlstd{,} \hlkwd{shuffle}\hlstd{(Data1}\hlopt{$}\hlstd{Year)))}
...
\hlstd{cl} \hlkwb{<-} \hlkwd{makeCluster}\hlstd{(}\hlkwd{detectCores}\hlstd{())}
\hlkwd{clusterExport}\hlstd{(cl,} \hlkwd{c}\hlstd{(}\hlstr{"Data1"}\hlstd{,} \hlstr{"year"}\hlstd{,} \hlstr{"shuffled.frame"}\hlstd{,} \hlstr{"mblm"}\hlstd{))}
\hlstd{mblm.mod} \hlkwb{<-} \hlkwd{parLapply}\hlstd{(cl,} \hlkwd{seq_len}\hlstd{(}\hlkwd{ncol}\hlstd{(year)),} \hlkwa{function}\hlstd{(}\hlkwc{i}\hlstd{)\{}
...
\hlkwd{abline}\hlstd{(model1s,} \hlkwc{col} \hlstd{=} \hlstr{"purple"}\hlstd{,} \hlkwc{add} \hlstd{=} \hlnum{TRUE}\hlstd{)}
