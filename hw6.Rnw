...
<<prob4o, dependson = "prob4">>=
hist(lm_perm, main = "Histogram of 10000 Permutations of \n Year and Resulting \n LM Slope Coefficients", xlab = expression(beta[Year]), ylab = "Count", breaks = 50)
abline(v = coef(lm.Year)[2], col = "purple")

perm_pvalue <- (length(which(lm_perm<=coef(lm.Year)[2])) + length(which(lm_perm>=-coef(lm.Year)[2])))/length(lm_perm)
@
...
cl <- makeCluster(detectCores())
clusterExport(cl, c("Data1", "year", "shuffled.frame", "mblm"))
mblm.mod <- parLapply(cl, seq_len(ncol(year)), function(i){
...
It is estimated that the median temperature in this data set decreased by \Sexpr{coef(model1s)[2]} $^\circ$F each year with an associated 95\% confidence interval of \Sexpr{CI[1]} to \Sexpr{CI[2]} $^\circ$F.
...
cl <- makeCluster(detectCores())
clusterExport(cl, c("Data1", "rand.noise"))
lm.noise <- parSapply(cl, seq_len(ncol(rand.noise)), function(x){
  # Get the pvalues!
  return(summary(lm(rand.noise[,x] ~ Year, data = Data1))$coefficients["Year","Pr(>|t|)"])
})
stopCluster(cl)
...
cl <- makeCluster(detectCores())
clusterExport(cl, c("Data1", "rand.noise", "noise.frame", "mblm"))
mblm.noise.reject <- parSapply(cl, seq_len(ncol(noise.frame)), function(i){
  noise.frame$out <- rand.noise[,i]
  # Is this where we use the CIs?
  CI<-confint(mblm(out~Year, data=noise.frame,repeated = F))[2,]
  return((0 < CI[1]) | (0 > CI[2])) # Is 0 in the CI?
})
stopCluster(cl)
...
cl <- makeCluster(detectCores())
clusterExport(cl, c("Data1", "shuffle", "rand.noise"))
# Loop for each simulated noise vector
lm.perm.pvals <- parSapply(cl, seq_len(ncol(noise.frame)), function(i){
  # Get the "observed result" from fitting the ith dataset
  lm.slope <- coef(lm(rand.noise[,i] ~ Year, data = Data1))[2]

  # Now loop to make a permutation distribution using this sim and suffled years
  lm.perms <- replicate(10000, coef(lm(rand.noise[,i] ~ shuffle(Year), data = Data1))[2])
...
# Note: We could combine these three simulations and have parSapply return a matrix of reject/not reject outcomes
@

lm $t$-test type I error rate: \Sexpr{lm.noise_type1}

mblm CI type I error rate: \Sexpr{mblm.noise_type1}

lm permutation test type I error rate: \Sexpr{lm.perm_type1}
...
and
$$\sigma^2_{0.6} = (1-0.3^2) \Sexpr{resid.se^2} = \Sexpr{(1-0.6^2) * resid.se^2}$$
