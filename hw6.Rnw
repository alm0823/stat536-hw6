drea\documentclass[11pt]{article}
...
<<prob1>>=
 ggplot(data = Data1, aes(x = Year, y = DaysBelow32)) + geom_point() +
  ...
  scale_y_continuous(breaks = c(seq(min(as.numeric(Data1$DaysBelow32)),max(as.numeric(Data1$DaysBelow32))+5, by = 10))) +
...
if(file.exists("prob5.Rdata")){
  ...
  stopCluster(cl)
  save(mblm.mod, file = "prob5.Rdata")
}
...
<<prob7b, echo = TRUE, dependson = "prob2">>=
...
if(file.exists("prob8p.Rdata")){
  ...
  lm.ar1.perm <- parCapply(cl, rand.ar1[,1:1000], function(x){
    ...
    # Return pvalue
    return(mean(abs(lm.slope) > abs(lm.ar1.perm)))
  })
  stopCluster(cl)
  save(lm.ar1.perm, file = "prob6p.Rdata")
}
