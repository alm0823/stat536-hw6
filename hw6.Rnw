...
The true mean proportion of days below freezing was associated with a decrease of \Sexpr{-1*coef(lm.Year)[2]} days per year in these years recorded at the spots in Bozeman data were collected.

Data were taken through an observational study and years were not randomly chosen, which is why we stated the conclusion using the wording ``associated" and ``in these years recorded at the spots in Bozeman data were collected", respectively. %as we teach our students to do...
...
perm_pvalue <- (length(which(lm_perm<=coef(lm.Year)[2])) + 
                  length(which(lm_perm>=-coef(lm.Year)[2]))) /
  length(lm_perm)
@
...
Based on 10000 permutations, the resulted in a two sided pvalue of \Sexpr{perm_pvalue}, from a test statistic of \Sexpr{round(coef(lm.Year)[2],4)}, which is also the estimated slope from a model with the observed data.
...
{\texttt lm()} gave an estimated slope coefficeint of \Sexpr{coef(lm.Year)[2]} and {\texttt mblm()} gave an estimated slope coefficient of \Sexpr{coef(model1s)[2]} which was found to be ``statistically significant" in the permutation test.

Using the same hypotheses from 4, but with the {\texttt mblm()} function fitting the model with the median response, the permutation test based on 10,000 iterations led to a two sided pvalue of \Sexpr{pvalue.mblm} using the test statistic of the observed {\texttt mblm()} slope coefficient (\Sexpr{coef(model1s)[2]}) and the null hypothesis of no relationship between year and temperature. The explicit hypotheses are stated below. There is strong evidence that year was associated with an effect on median temperature in these data. 

$H_{o}$: $\beta_{Year.median}$ = 0

$H_{a}$: $\beta_{Year.median} \neq$ 0

It is estimated that the median temperature in this data set decreased by \Sexpr{-1*coef(model1s)[2]} $^\circ$F each subsequent year with an associated 95\% confidence interval of decreasing by \Sexpr{-1*CI[1]} to \Sexpr{-1*CI[2]} $^\circ$F each subsequent year.

However, I would question the validity of the confidence level set using {\texttt confint()} on an {\texttt mblm()} object. If there was strong evidence of an effect of temperature, at the 95\% confidence level, we would expect to ``reject" about 95\% of the time in the long run, meaning that approximately 95\% of confidence intervals, in the long run, should not contain 0. In this problem, we rejected the null, or found strong evidence that the 0 is not a plausible slope coefficient and the percentage of confidence intervals not containing 0 in 10,000 permutations using {\texttt mblm()} was \Sexpr{100-} {\bf Extract the pvalue I had saved for this..... not sure how to do.}
...
Based on 10,000 iterations, we came up with the following type I error rates:
...
We see that the permutation test is resistent to the induced correlation structure while the theoretical test and CI are not. The permutation test does not have the independent error assumptions required by the theoretical test and CI, resulting in the increased error rate.
