...
<<prob1>>=
 ggplot(data = Data1, aes(x = Year, y = DaysBelow32)) + geom_point() + scale_x_continuous(breaks = c(seq(min(as.numeric(Data1$Year)),max(as.numeric(Data1$Year))+5, by = 5))) + scale_y_continuous(breaks = c(seq(min(as.numeric(Data1$DaysBelow32)),max(as.numeric(Data1$DaysBelow32))+5, by = 10))) + theme_bw() + theme(axis.text.x = element_text(angle = 45, hjust = 1)) + labs(title = 'Bozeman Weather Data') + ylab(expression('Days Below 32' *~degree*F))
  
ggplot(data = Data1, aes(x = Year, y = PropDays)) + geom_point() + scale_x_continuous(breaks = c(seq(min(as.numeric(Data1$Year)),max(as.numeric(Data1$Year))+5, by = 5))) + scale_y_continuous(limits = c(0,1), breaks = c(seq(0,1, by = 0.1))) + theme_bw() + theme(axis.text.x = element_text(angle = 45, hjust = 1)) + labs(title = 'Bozeman Weather Data') + ylab(expression('Proportion of Days Below 32' *~degree*F))
...
The true mean proportion of days below freezing was associated with a decrease of \Sexpr{coef(lm.Year)[2]} days per year in these years recorded at the spots in Bozeman data were collected.
...
hist(lm, main = "Histogram of 10000 Permutations of \n Year and Resulting \n LM Slope Coefficients", xlab = expression(beta[Year]), ylab = "Count")
abline(v = coef(lm.Year)[2], col = "purple")

perm_pvalue <- (length(which(lm<=coef(lm.Year)[2])) + length(which(lm>=-coef(lm.Year)[2])))/length(lm)
@
...
Based on 10000 permutations, the resulted in a pvalue of \Sexpr{perm_pvalue}, from a test statistic of \Sexpr{round(coef(lm.Year)[2],4)}, which is also the estimated slope from a model with the observed data.
...
<<prob5,warning=F,message=F, cache = TRUE>>=
...
pvalue.mblm <- length(c(which(mblm.coeff<=model1s.coef), which(mblm.coeff>=-model1s.coef)))/length(mblm.coeff)
...
#for(i in 1:10000){
  #dec.mblm[i] <- ifelse(confint(mblm.mod[[i]])[2,1]<0 & confint(mblm.mod[[i]])[2,2]>=0, "R", "FTR")
#}

cl.mblm.confint <- length(which(as.factor(dec.mblm) == "R"))/length(dec.mblm)

cl.mblm.confint.save <- 0.3476
@

{\texttt lm()} gave an estimated slope coefficeint of \Sexpr{coef(lm.Year)[2]} and {\texttt mblm()} gave an estimated slope coefficient of \Sexpr{coef(model1s)[2]}.

Using the same hypotheses from 4, but with the {\texttt mblm()} function fitting the model with the median response, the permutation test based on 10,000 iterations led to a pvalue of \Sexpr{pvalue.mblm} using the test statistic of the observed slope coefficient (\Sexpr{coef(model1s)[2]}) and the null hypothesis of no relationship between year and temperature. The explicit hypotheses are stated below. There is strong evidence that year had an effect on median temperature in these data. 
$H_{o}$: $\beta_{Year.median}$ = 0

$H_{a}$: $\beta_{Year.median} \neq$ 0

It is estimated that the median temperature in this data set decreased by \Sexpr{coef(model1s)[2]} $\degrees$F each year with an associated 95\% confidence interval of \Sexpr{CI[1]} to \Sexpr{CI[2]} $\degrees$F.

However, I would question the validity of the confidence level set using {\texttt confint()} on an {\texttt mblm()} object. If there was strong evidence of an effect of temperature, at the 95\% confidence level, we would expect to ``reject" 5\% of the time, meaning that approximately 5\% of confidence intervals, in the long run, should contain 0. We ran 10,000 simulations and \Sexpr{cl.mblm.confint*100}\% contained 0. Something is wrong.
...
{\bf I don't understand what the permutation test is for. I can do a permutation test, but I don' t know what he wants us to test.}

<<prob6>>=

# first is lm
resid.se <- summary(lm.Year)$sigma

rand.noise <- data.frame(replicate(1000, rnorm(109,0,resid.se)))
dim(rand.noise)
lm.noise <- lapply(1:1000, function(x){lm(rand.noise[,x] ~ Data1$Year)})

lm.noise_type1 <- (length(which(lm.noise<=coef(lm.Year)[2])) + length(which(lm.noise>=-coef(lm.Year)[2])))/length(lm.noise)

summary(lm.noise[1])$coef[2,4]

# Above is not the type I error rate, I  don't know how to get it

# next is mblm
rand.noise$Year <- Data1$Year

noise.frame <- data.frame(Year = Data1$Year)
mblm.noise <- NULL
mblm.coeff.noise <- numeric(1000)
mblm.noise <- lapply(1:1000, function(i){
  noise.frame$out <- rand.noise[,i]
  return(mblm(out~Year, data=noise.frame,repeated = F))
})

# finally the permutation test, but I don't know what we should be testing
...
$\sigma^2_{0.03}$ = $\frac{\Sexpr{resid.se^2}}{1-0.03}  = \frac{\Sexpr{resid.var}{0.97} = 0.0012$

$\sigma^2_{0.06}$ = $\frac{\Sexpr{resid.se^2}}{1-0.06}  = \frac{\Sexpr{resid.var}}{0.94} = 0.0011$
...
<<prob7b, warning=F,message=F, results = 'hide'>>=
#change sd to make squared residual se

ar1sim<-arima.sim(n=2000,model=list(ar=c(0.6)),sd=5)
var(ar1sim)

ar1sim<-arima.sim(n=2000,model=list(ar=c(0.3)),sd=5)
var(ar1sim)
...
\item%10
{\it Make a plot of the original proportion response time series with the parametric linear, Theil-Sen, and Zhang methods$/$models on the same plot. You may want to use \verbatim{$plot(y~x,type="l")$} and then add lines to the plot.}

Estimates from all three functions visually are very similar.


<<prob10>>=
plot(PropDays ~ Year, data = Data1, type = "l", ylim = c(0,1), ylab = expression('Proportion of Days Below 32' *~degree*F))
abline(lm.Year)
abline(model1s, col = "purple", add = TRUE)
abline(a = zyp.Year$coefficients[1], b = zyp.Year$coefficients[2], col = "red")
